...
public class BlockValue extends Value
{
    ...
    public static BlockValue fromString(String str)
    {
        try
        {
            Identifier blockId = Identifier.parse(new StringReader(str));
            //if (Registry.BLOCK.func_212607_c(blockId))
            //{
            Block block = null;
                block = Registry.BLOCK.getOrEmpty(blockId).orElseThrow(() -> {
    ...
    BlockValue(BlockState state, World world, BlockPos position)
    {
        this.world = world;
        blockState = state;
        pos = position;
